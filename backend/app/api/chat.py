...
def chat_stream():
    ...
    terminal ``{"type":"done", sessionId, reply, actions, message}`` (or
    ``{"type":"error","error"}``). Same single-commit / rollback model as ``chat``;
    ALL writes happen inside the generator so the session INSERT and the message
    INSERTs share one transaction within the streaming context."""
    data = request.get_json(force=True) or {}
    message = str(data.get("message") or "").strip()
    if not message:
        return jsonify({"error": "message is required"}), 422
