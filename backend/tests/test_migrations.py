...
# ---- 0017: recipe_ingredients.free_text ----
...
def test_0017_adds_free_text_to_a_real_pre_0017_database(tmp_path):
    ...
    free_text — and 0017's `_has_column` guard then makes it a no-op. Asserting
    ...
    A genuine pre-0017 install is simulated by dropping the column back off,
    ...
    db = str(tmp_path / "m.db")
    assert _run_alembic(db, "upgrade", "0016_hot_fk_indexes").returncode == 0
    c = sqlite3.connect(db)
    c.execute("PRAGMA foreign_keys=ON")
    _insert(c, "groups", {"id": "g", "name": "G"})
    _insert(c, "recipes", {"id": "r", "group_id": "g", "name": "R", "slug": "r"})
    ...
    assert col is not None, "0017 did not add the column"
...
def test_0017_downgrade_then_upgrade_round_trips_with_rows_present(tmp_path):
...
def test_0017_migrated_and_create_all_agree_on_free_text(tmp_path):
