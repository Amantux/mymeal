...
def test_default_user_password_is_not_a_known_login_backdoor(app):
    # Regression (the missed third sibling — Edibl and HomeHoard were fixed in
    # August): the account _default_user()/_ingress_user() synthesize must NOT be
    # reachable via a plain password login. It is minted at startup on every
    # Supervisor-run install (open OR hardened) so the integration token has a
    # household to bind to, and a fixed literal password made it a public,
    # guessable owner login on every install.
    ensure_integration_token(app)
    client = app.test_client()

    resp = client.post(
        "/api/v1/users/login",
        json={"username": "local@mymeal", "password": "unused"},
    )
...
def test_preexisting_backdoor_password_is_rotated_on_upgrade(app):
    # Installs that ran BEFORE this fix already carry a local@mymeal row with the
    # literal "unused" password. Rotating only the create branch would leave
    # every such upgrade exposed forever, since _default_user() just returns the
    # existing row. Simulate the pre-existing state and confirm it heals.
