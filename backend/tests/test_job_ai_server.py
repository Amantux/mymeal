...
def test_an_unset_area_still_means_same_as_chat(noauth_app):
    ...
    with app.app_context():
        assert resolve_job_provider("nutrition", _gid(app)) is None


def test_a_job_can_point_at_its_own_server(noauth_app):
    app = noauth_app
    c = app.test_client()
    ...
    with app.app_context():
        ...
        p = resolve_job_provider("nutrition", _gid(app))
        assert p.host == "http://192.168.1.50:11434"
        assert p.model == "qwen3:4b"


def test_the_async_server_does_not_leak_into_chat(noauth_app):
...
def test_the_two_job_areas_are_independent(noauth_app):
    app = noauth_app
    c = app.test_client()
    c.put("/api/v1/ai/job-settings", json={
...
def test_a_per_run_option_beats_the_stored_server(noauth_app):
    app = noauth_app
    c = app.test_client()
    c.put("/api/v1/ai/job-settings", json={
...
def test_a_blank_apikey_on_resave_keeps_the_stored_one(noauth_app):
...
def test_clearing_the_async_key_is_explicit(noauth_app):
    app = noauth_app
    c = app.test_client()
    ...
    with app.app_context():
        assert job_override(_gid(app), "nutrition")["api_key"] is None
...
def test_an_unsafe_server_supplied_per_run_is_refused_at_use(noauth_app):
    ...
    with app.app_context():
        with pytest.raises(ProviderError) as ei:
            resolve_job_provider("nutrition", _gid(app),
