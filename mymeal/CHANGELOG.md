...
- Address the integration review's two should-fix findings.
...
- Close the last raw-exception leak in extract_json.
...
- Medium/Nits sweep — leaks, 500s, wrong results, N+1s, indexes.

## 1.2.36

- FK-safe SQLite rebuild in 0013 downgrade.
...
- Register RATELIMIT_ENABLED in gen_config_docs GROUPS.
...
- Scope ingredient foodId/unitId to the group; harden register.

## 1.2.30

- Close the login user-enumeration timing oracle.

## 1.2.29

- Ingress-keyed frame headers, safe_join image path, secret-key perms.

## 1.2.28

- Alias policy hardening — reviewer round.

## 1.2.27

- "Tidy up ingredients" — propose duplicate-food merges.

## 1.2.26

- Carry the qualifier through the editor.

## 1.2.25

- A home for the variety, and for the material boundary.

## 1.2.24

- Rank a name against the household's own foods.
...
- Upgrade to PostgreSQL 18, migrating existing clusters.
...
- Close two proven bypasses in the MCP debug surface.

## 1.2.9

- Drop the undeclared *_PROC env var caught by the config contract.
...
- Recipe + experiment services, and an integration test harness.
...
- Run app processes as a non-root user.
...
- Streaming is a Settings toggle only — remove the per-widget toggle.

## 1.1.68

- Hide the [[REPORT_BUG]] marker while streaming + guard it with a test.

## 1.1.67

- Report-a-bug walkthrough in the assistant.

## 1.1.66

- Pick the background-task model from a selectable dropdown.

## 1.1.65

- Pick a model (SLM) for background tasks from a list.

## 1.1.64

- Separate AI provider/model for background jobs.

## 1.1.63

- Easier Ollama setup — auto-list models, widget setup state, SSRF guard.

## 1.1.62

- Bundle a custom AppArmor profile.

## 1.1.61

- Surface streaming commit failures + drop empty bubble on transport error.
...
- Real streaming chat responses (opt-in stream vs POST).

## 1.1.58

- Report-a-bug top-bar action with scrubbed diagnostics.
...
- Expose OPENAI_BASE_URL + AI_TIMEOUT_SECONDS as HA add-on options (Phase 5).
...
- Register OLLAMA_SEARCH_KEY in the config-docs generator.
...
- Import a recipe by name via Ollama web search.

## 1.1.48

- Add a Database page under Utilities (parity with Edibl/HomeHoard).
...
- Assistant can scrape/import recipes from a URL.

## 1.1.45

- Batch count for recipe components steps in 0.5 increments.
...
- Structured reorderable step editor + verify ingredient round-trip.
...
- Email a recipe (mailto Markdown) + copy as Markdown.

## 1.1.37

- Address reviewer findings on the feature batch.
...
- Medications / vitamins module (dose + frequency, per-user).

## 1.1.32

- Clamp/cap recipe-builder inputs (address review).
